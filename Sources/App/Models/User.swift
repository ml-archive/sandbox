...
struct Authentication {
    static let AccessTokenSigningKey: Bytes = Array("NODES".utf8)
...
extension User {
    func generateToken() throws {
        // Generate our Token
        let jwt = try JWT(payload: Node(ExpirationTimeClaim(Authentication.generateExpirationDate())), signer: HS256(key: Authentication.AccessTokenSigningKey))
